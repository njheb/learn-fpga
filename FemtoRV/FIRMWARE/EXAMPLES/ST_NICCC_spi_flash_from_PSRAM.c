...
//#define PSRAM_BASE ((uint32_t*)(0xC00000))
#define PSRAM_BASE ((uint32_t*)(0x200000))
...
//uint32_t* sram_scene = PSRAM_BASE ; //RV32_SRAMDATA; //0x20000 all 512K words
//uint32_t* sram_scene = PSRAM_BASE+AVOID_RAM_ZONE_VARS ; //RV32_SRAMDATA; //0x20000 all 512K words
uint32_t* sram_scene = PSRAM_BASE; //RV32_SRAMDATA; //0x200000 

void transfer_scene_to_sram(void) RV32_FASTCODE;
void transfer_scene_to_sram(void)
{
//   for (uint32_t i = 0; i<0x10000; i++)  //64KB*4=256KB
...
uint8_t next_spi_byte() {
   ...
   if(spi_word_addr != spi_addr >> 2) {
      ...
      spi_u.spi_word = sram_scene[spi_word_addr];
...
int main() {
    ...
    transfer_scene_to_sram();
#if 0
    for (int i = 0; i<10; i++)
    {
      printf("%d: %x %x\n", i, sram_scene[i], SPI_FLASH_BASE[i+0x40000]);
    }
#endif    
