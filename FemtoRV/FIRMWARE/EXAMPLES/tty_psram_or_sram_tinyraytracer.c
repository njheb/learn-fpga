//where there are no enough pmods for psram testing and oled display
//use low resolution tty output
//temporary measure use SRAM infrastructure to access PSRAM to check
//will need you to adjust .ld temporarily
...
static inline float max(float x, float y) RV32_SRAMCODE_INLINE;
static inline float min(float x, float y) RV32_SRAMCODE_INLINE;
static inline void graphics_init(void) RV32_SRAMCODE_INLINE;
static inline void graphics_terminate(void) RV32_SRAMCODE_INLINE;
void graphics_set_pixel(int x, int y, float r, float g, float b) RV32_SRAMCODE_INLINE;
static inline stats_begin_frame(void) RV32_SRAMCODE_INLINE;
static inline stats_begin_pixel(void) RV32_SRAMCODE_INLINE;
static inline stats_end_pixel(void) RV32_SRAMCODE_INLINE;
static inline stats_end_frame(void) RV32_SRAMCODE_INLINE;
static inline vec3 make_vec3(float x, float y, float z) RV32_SRAMCODE_INLINE;
static inline vec4 make_vec4(float x, float y, float z, float w) RV32_SRAMCODE_INLINE;
static inline vec3 vec3_neg(vec3 V) RV32_SRAMCODE_INLINE;
static inline vec3 vec3_add(vec3 U, vec3 V) RV32_SRAMCODE_INLINE;
static inline vec3 vec3_sub(vec3 U, vec3 V) RV32_SRAMCODE_INLINE;
static inline float vec3_dot(vec3 U, vec3 V) RV32_SRAMCODE_INLINE;
static inline vec3 vec3_scale(float s, vec3 U) RV32_SRAMCODE_INLINE;
static inline float vec3_length(vec3 U) RV32_SRAMCODE_INLINE;
static inline vec3 vec3_normalize(vec3 U) RV32_SRAMCODE_INLINE;
Light make_Light(vec3 position, float intensity) RV32_SRAMCODE;
Material make_Material(float r, vec4 a, vec3 color, float spec) RV32_SRAMCODE;
Material make_Material_default(void) RV32_SRAMCODE;
Sphere make_Sphere(vec3 c, float r, Material M) RV32_SRAMCODE;
BOOL Sphere_ray_intersect(Sphere *S, vec3 orig, vec3 dir, float *t0) RV32_SRAMCODE;
vec3 reflect(vec3 I, vec3 N) RV32_SRAMCODE;
vec3 refract(vec3 I, vec3 N, float eta_t, float eta_i) RV32_SRAMCODE;
BOOL scene_intersect(vec3 orig, vec3 dir, Sphere *spheres, int nb_spheres, vec3 *hit, vec3 *N, Material *material) RV32_SRAMCODE;
vec3 cast_ray(vec3 orig, vec3 dir, Sphere *spheres, int nb_spheres, Light *lights, int nb_lights, int depth) RV32_SRAMCODE;
void render(Sphere *spheres, int nb_spheres, Light *lights, int nb_lights) RV32_SRAMCODE;
...
#if 1
    //we are using serial tty as our pmod is taken up with tinytapeout psram module
    printf("\033[H");
    printf("\033[49m\n");
#else
...
void graphics_set_pixel(int x, int y, float r, float g, float b) {
   ...
   b = max(0.0f, min(1.0f, b));
	...
	uint8_t B = (uint8_t)(255.0f * b);
        ...
        //printf("[48;2;%d;%d;%dm  \n",R,G,B);
        if (x >= (graphics_width-1)) printf("\033[49m\n");
#else
   switch(FGA_mode) {
      case GL_MODE_OLED: {
	uint8_t R = (uint8_t)(255.0f * r);
...
//uint32_t frame_ticks; //overflowing with psram testing
